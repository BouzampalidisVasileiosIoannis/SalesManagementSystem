...
public class ReceiptSelectionDialog extends JDialog {
	...
	private ReceiptInputWindow inputDialog;
	private SalesManager selectedAgent;
	@SuppressWarnings("unused")
	private String fileType;
	
	private Receipt receipt;
	
	public ReceiptSelectionDialog(ReceiptInputWindow dialog, SalesManager agent, String fileTypeFlag) {
		inputDialog = dialog;
		selectedAgent = agent;
	...
	public void initialise(){
		...
		try{
			...
			JOptionPane.showMessageDialog(null,"Couldn't Load Profile, Try Again!");
	...
	//FIXING  MULTIPLE IF-ELSE STATEMENTS OF okButtonPressed METHOD BY ADDING THIS METHOD
	private int calculateSalesValue(boolean isSelected, double d) {
		if (isSelected) {
	        return (int) d;
	    } else {
	        return -1;
	...
	//FIXING  MULTIPLE IF-ELSE STATEMENTS OF okButtonPressed METHOD BY ADDING THIS METHOD
		private int calculateItemsValue(boolean isSelected, double d) {
			if (isSelected) {
		        return (int) d;
		    } else {
		        return -1;
	...
	//FIXED MULTIPLE IF-ELSE STATEMENTS
	protected void okButtonPressed(ActionEvent evt) {
	    totalSales = calculateSalesValue(totalSalesCheckBox.isSelected(), selectedAgent.calculateTotalSales());
	    totalItems = calculateItemsValue(totalItemsCheckBox.isSelected(), selectedAgent.calculateTotalItems());
	    shirtSales = calculateSalesValue(shirtRadio.isSelected(), selectedAgent.calculateKindSales(ClothesKind.SHIRT.getValue()));
	    skirtSales = calculateSalesValue(skirtRadio.isSelected(), selectedAgent.calculateKindSales(ClothesKind.SKIRT.getValue()));
	    coatsSales = calculateSalesValue(coatRadio.isSelected(), selectedAgent.calculateKindSales(ClothesKind.COAT.getValue()));
	    trousersSales = calculateSalesValue(trousersRadio.isSelected(), selectedAgent.calculateKindSales(ClothesKind.TROUSER.getValue()));
	    commission = calculateSalesValue(commissionCheckBox.isSelected(), selectedAgent.calculateCommission());

	    ReportResultWindow rs = new ReportResultWindow(this, selectedAgent, totalSales, totalItems, shirtSales, skirtSales, trousersSales, coatsSales, commission);
	    rs.setVisible(true);
	    this.setVisible(false);
	}
	
	//FIXING COMPLEX CONDITIONAL STATEMENT OF addReceiptButtonPressed METHOD  BY ADDING THIS METHOD
	private boolean emptyFields() {
		return receiptIDTextField.getText().isEmpty() && dateTextField.getText().isEmpty() 
	...
	//FIXED COMPLEX CONDITIONAL STATEMENT
	private void addReceiptButtonPressed(ActionEvent evt) {
		if(emptyFields()){
			JOptionPane.showMessageDialog(null,"Fields Are Empty, Try Again!");
	...
	//SIMPLYFYING THE ALGORITHM OF THE appendFile
	private void appendFile(){
		selectedAgent.getFileAppender().setReceiptInfo(receipt);
	...
	private void addReceipt(){
		...
		if(kindTextField.getText().equals("Shirts") || kindTextField.getText().equals("Shirt"))		
			receipt.setKind(ClothesKind.SHIRT.getValue());
		else if (kindTextField.getText().equals("Skirts") || kindTextField.getText().equals("Skirt"))
			receipt.setKind(ClothesKind.SKIRT.getValue());
		else if (kindTextField.getText().equals("Trousers") || kindTextField.getText().equals("Trouser"))
			receipt.setKind(ClothesKind.TROUSER.getValue());
		else if(kindTextField.getText().equals("Coats") || kindTextField.getText().equals("Coat"))				
		...
		try{
			...
			JOptionPane.showMessageDialog(null,"Receipt Addded Successfully");

		}catch (NumberFormatException e){
			JOptionPane.showMessageDialog(null,"Could Not Add Receipt");
