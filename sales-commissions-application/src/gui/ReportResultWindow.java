...
public class ReportResultWindow extends JDialog {
	...
	private ReceiptSelectionDialog receiptSelectionDialog;
	...
	public ReportResultWindow(final ReceiptSelectionDialog sw, SalesManager agent,double tSales,int tItems,
			float shirtS,float skirtS,float trousersS,float coatsS,double com) {
		receiptSelectionDialog = sw;
		selectedAgent = agent;
		totalSales = tSales;
		totalItems = tItems;
		shirtSales = shirtS;
	...
	private void initialise(){
		...
		resultWindowPanel.setLayout(null);
		{
			...
			xmlReportButton.setBounds(436, 150, 163, 84);
		...
		}
		{
			...
			htmlReportButton.setBounds(436, 268, 163, 84);
	...
	//UPDATED METHOD FOR SAVING A FILE SELECTED BY USER  
	private void outputTXTButtonPressed(ActionEvent evt) {
	    ...
	    if (userSelection == JFileChooser.APPROVE_OPTION) {
	        ...
	        TXTReportGenerator makeTXTFile = new TXTReportGenerator(selectedAgent, fileToSave);
	        makeTXTFile.saveFile();
	        JOptionPane.showMessageDialog(null, "TXT File saved successfully.");
	    }
	}
	...
	private void outputXMLButtonPressed(ActionEvent evt) {
	    ...
	    if (userSelection == JFileChooser.APPROVE_OPTION) {
	        ...
	        XMLReportGenerator makeXMLFile = new XMLReportGenerator(selectedAgent, fileToSave);
	...
	private void outputHTMLButtonPressed(ActionEvent evt) {
	    ...
	    if (userSelection == JFileChooser.APPROVE_OPTION) {
	        ...
	        HTMLReportGenerator makeHTMLFile = new HTMLReportGenerator(selectedAgent, fileToSave);
	        makeHTMLFile.saveFile();
	        JOptionPane.showMessageDialog(null, "HTML File saved successfully.");
	    }
	}
	...
	private void cancelButtonPressed(ActionEvent evt) {
		receiptSelectionDialog.setVisible(true);
		this.dispose();	
	}
