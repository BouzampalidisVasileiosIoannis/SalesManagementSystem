...
public class XMLReportGenerator extends ReportGenerator{	
	...
	//UPDATED CONSTRUCTOR
	public XMLReportGenerator(SalesManager a, File file){
			agent = a;
			fileToSave = file;
	}	
	...
	public void writeSalesData() {
		...
		trouserSales.appendChild(document.createTextNode(Float.toString(agent.calculateKindSales(ClothesKind.TROUSER.getValue()))));
		agentElem.appendChild(trouserSales);
		 
		Element skirtsSales = document.createElement("SkirtsSales");
		skirtsSales.appendChild(document.createTextNode(Float.toString(agent.calculateKindSales(ClothesKind.SKIRT.getValue()))));
		agentElem.appendChild(skirtsSales);
		 
		Element shirtsSales = document.createElement("ShirtsSales");
		shirtsSales.appendChild(document.createTextNode(Float.toString(agent.calculateKindSales(ClothesKind.SHIRT.getValue()))));
		agentElem.appendChild(shirtsSales);
		 
		Element coatsSales = document.createElement("CoatsSales");
		coatsSales.appendChild(document.createTextNode(Float.toString(agent.calculateKindSales(ClothesKind.COAT.getValue()))));
